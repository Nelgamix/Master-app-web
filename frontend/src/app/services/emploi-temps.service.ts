...
export class EmploiTempsService {
  ...
  updateSingleWeek(semaine: number, annee: number, cb?: Function): Observable<Semaine> {
    return Observable.create(obs => {
      ...
      if (!t) {
        const p = {params: new HttpParams().set('from_year', annee.toString()).set('from_week', semaine.toString())};
        this.http.get('backend/ical', p)
          .subscribe(
  ...
  updateAllWeeks(cb?: Function): Observable<Semaine[]> {
    return Observable.create(obs => {
      const p = {params: new HttpParams().set('info', '')};
      this.http.get('backend/ical', p).subscribe(data => this.loadData(data, obs, true, cb));
    });
  }
